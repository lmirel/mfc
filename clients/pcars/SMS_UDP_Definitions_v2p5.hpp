...
typedef struct ssTelemetryData 
{
      unsigned int	sPacketSize;// = 559;
...
typedef struct ssRaceData
{
	unsigned int	sPacketSize ;//= 308;
	PacketBase						sBase;												// 0 12
...
typedef struct ssParticipantsData
{
	unsigned int	sPacketSize ;//= 1136;
	PacketBase					sBase;														//	0 	12
...
#pragma pack(push)
#pragma pack(1)
typedef struct ssParticipantInfo
{
...
typedef struct ssTimingsData
{
	unsigned int						sPacketSize ;//= 1063;
	PacketBase						sBase;													// 0 12
...
typedef struct ssGameStateData
{
	unsigned int								sPacketSize ;//= 24;
	PacketBase												sBase;						//10
...
typedef struct ssParticipantStatsInfo
{
...
}sParticipantStatsInfo;																					// 32 -- padded to  32
 

typedef struct ssParticipantsStats
{
	sParticipantStatsInfo	sParticipants[UDP_STREAMER_PARTICIPANTS_SUPPORTED]; //1024
}sParticipantsStats;

typedef struct ssTimeStatsData
{
	unsigned int		sPacketSize ;//= 1024;
	PacketBase						sBase;											// 0 12
...
typedef struct ssVehicleInfo
{
	unsigned short				sIndex; // 0 2
	unsigned int				sClass; // 2 6 
	char						sName[VEHICLE_NAME_LENGTH_MAX]; // 6 70
}sVehicleInfo; // padded to 72

		
typedef struct ssParticipantVehicleNamesData
{
	unsigned int	sPacketSize ;//= 1164;
	PacketBase					sBase; // 0 12
	sVehicleInfo				sVehicles[VEHICLES_PER_PACKET]; //12 16*72
}sParticipantVehicleNamesData;	// 1164

typedef struct ssClassInfo
{
	unsigned int				sClassIndex; // 0 4 
	char						sName[CLASS_NAME_LENGTH_MAX]; // 4 24
}sClassInfo;

typedef struct ssVehicleClassNamesData
{
	unsigned int	sPacketSize ;//= 1452;
	PacketBase					sBase; // 0 12
	sClassInfo					sClasses[CLASSES_SUPPORTED_PER_PACKET]; //12 24*60
}sVehicleClassNamesData;				 			// 1452
